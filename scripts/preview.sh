...
# The local Hugo server rebuilds generated HTML whenever a source file changes,
# overwriting the formatting previously applied by Prettier. Unlike the one-time
# GitHub Actions build, the local preview therefore needs to run Prettier after
# the initial build and after every subsequent rebuild. The marker prevents the
# watcher from reacting repeatedly to the changes made by Prettier itself.
watch_generated_html() {
    local marker="$HUGO_DIR/.prettier-last-run"
    local generated_html="$HUGO_DIR/public/index.html"

    while [[ ! -f "$generated_html" ]]; do
        sleep 0.1
    done
...
# Stop the background watcher together with the Hugo server so that it does not
# remain active after the preview exits or the user presses Ctrl-C.
stop_html_watcher() {
    if [[ -n "${HTML_WATCHER_PID:-}" ]]; then
